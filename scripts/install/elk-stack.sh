...
# Silent execution function (fallback if not provided by ProxmoxVE)
if ! type silent >/dev/null 2>&1; then
    silent() {
        "$@" >/dev/null 2>&1
    }
fi
...
if ! $STD apt-get install -qq -y elasticsearch logstash kibana; then
...
$STD /usr/share/logstash/bin/logstash-keystore --path.settings /etc/logstash create
...
# ----------------------------------------------------------------------------
step_start "Writing Kibana Configuration"
...
step_done "Wrote Kibana Configuration"

step_start "Creating Kibana Enrollment Token"
...
step_done "Created Kibana Enrollment Token"

step_start "Applying Kibana Enrollment Token"
if ! $STD /usr/share/kibana/bin/kibana-setup --enrollment-token "$ENROLLMENT_TOKEN"; then
    msg_error "Failed to apply enrollment token to Kibana"
    exit 1
fi
step_done "Applied Kibana Enrollment Token"

# Update Elasticsearch connection to use localhost instead of IP
step_start "Updating Kibana Elasticsearch Hosts"
sed -i 's|elasticsearch.hosts:.*|elasticsearch.hosts: [https://localhost:9200]|' /etc/kibana/kibana.yml
step_done "Updated Kibana Elasticsearch Hosts"
...
LOGSTASH_KEY_RESPONSE=$(curl $CURL_OPTS -s -X POST \
    ...
    -u "elastic:$ELASTIC_PASSWORD" -H "Content-Type: application/json" -d "$LOGSTASH_ROLE")
set -e
...
step_start "Adding Logstash API key to Keystore"
echo "$LOGSTASH_API_KEY" | /usr/share/logstash/bin/logstash-keystore --path.settings /etc/logstash add ELASTICSEARCH_API_KEY --stdin >/dev/null 2>&1
step_done "Added Logstash API key to Keystore"

step_start "Setting Keystore Ownership and Permissions"
chown logstash:root /etc/logstash/logstash.keystore
chmod 0600 /etc/logstash/logstash.keystore
step_done "Set Keystore Ownership and Permissions"
...
# ----------------------------------------------------------------------------
step_start "Starting Logstash and Kibana Services"
$STD systemctl enable logstash kibana

$STD systemctl start logstash kibana

$STD systemctl status logstash --no-pager
$STD systemctl status kibana --no-pager
step_done "Started Logstash and Kibana Services"
# ----------------------------------------------------------------------------
# Create Kibana Data View
# ----------------------------------------------------------------------------
step_start "Waiting for Kibana to be ready"
MAX_WAIT=90
WAITED=0
...
if [ $WAITED -ge $MAX_WAIT ]; then
    msg_error "Kibana did not become ready in time, skipping data view creation"
    exit 1
else
    step_start "Creating default data views"
    DATAVIEW_CREATED=false
    for attempt in 1 2 3; do
        # Temporarily disable error exit for command substitution
        set +e
    ...
    if [ "$DATAVIEW_CREATED" = false ]; then
        msg_error "Data view creation failed or already exists"
        exit 1
    fi
fi
...
# Test Log Ingestion
# ----------------------------------------------------------------------------
step_start "Waiting for Logstash to be ready"
...
if [ $LOGSTASH_WAITED -ge $MAX_LOGSTASH_WAIT ]; then
    msg_error "Logstash HTTP input not ready, skipping ingestion test"
    exit 1
else
    step_start "Sending test log to Logstash"
...
EOF
    ...
    if echo "$SEARCH_RESPONSE" | jq -r '.hits.total.value' | grep -qE '^[1-9][0-9]*$'; then
        step_done "Test log ingested and indexed successfully"
    else
        msg_error "Failed to ingest test log"
        exit 1
    fi
fi
...
msg_ok "ELK Stack installation completed successfully"