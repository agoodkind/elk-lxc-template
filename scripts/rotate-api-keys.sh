...
if grep -q "xpack.security.http.ssl.enabled: true" \
    "$ES_CONFIG"; then
...
OLD_KEY_ID=$(curl $CURL_OPTS -s -X GET \
    "$ES_URL/_security/api_key" \
    -u "elastic:$ELASTIC_PASSWORD" \
    -H "Content-Type: application/json" \
    | grep -o '"id":"[^"]*","name":"logstash_writer"' \
    | cut -d'"' -f4 | head -1)
if [ -n "$OLD_KEY_ID" ]; then
    curl $CURL_OPTS -s -X DELETE \
        "$ES_URL/_security/api_key" \
        -u "elastic:$ELASTIC_PASSWORD" \
        -H "Content-Type: application/json" \
        -d "{\"ids\":[\"$OLD_KEY_ID\"]}" > /dev/null
...
unset ELASTIC_PASSWORD LOGSTASH_API_KEY
unset LOGSTASH_KEY_RESPONSE OLD_KEY_ID LOGSTASH_ROLE
